...
class HyperliquidAPI:
    ...
    def __init__(self):
        ...
        # Store main wallet address for querying balances (if provided)
        # API wallet is used for signing, but balances are queried from main wallet
        self.main_wallet_address = CONFIG.get("hyperliquid_wallet_address")
        ...
        logging.info(f"🔑 Hyperliquid API initialized:")
        logging.info(f"   - API Wallet (for signing): {self.wallet.address}")
        if self.main_wallet_address:
            logging.info(f"   - Main Wallet (for balances): {self.main_wallet_address}")
        else:
            logging.info(f"   - Main Wallet: Not set, using API wallet for queries")
        logging.info(f"   - Network: {network}, Base URL: {self.base_url}")
    ...
    async def place_buy_order(self, asset, amount, slippage=0.01):
        ...
        logging.info(f"🔄 Placing BUY order: {asset}, size: {amount}, slippage: {slippage}")
        
        result = await self._retry(lambda: self.exchange.market_open(asset, True, amount, None, slippage))
        
        # Log response for debugging
        logging.info(f"📋 BUY order response: {result}")
        
        # Check for errors in response
        if isinstance(result, dict):
            status = result.get("status")
            if status == "ok":
                # Try to extract order info for logging
                try:
                    ...
                    if isinstance(response_data, dict):
                        ...
                        if statuses and isinstance(statuses[0], dict):
                            ...
                            if "resting" in status_obj:
                                oid = status_obj.get("resting", {}).get("oid")
                                logging.info(f"✅ BUY order placed (resting): {asset}, order_id: {oid}")
                            elif "filled" in status_obj:
                                filled = status_obj.get("filled", {})
                                logging.info(f"✅ BUY order FILLED: {asset}, size: {filled.get('totalSz')}, avg_price: {filled.get('avgPx')}")
                            elif "error" in status_obj:
                                error_msg = status_obj.get("error", "Unknown error")
                                logging.error(f"❌ BUY order error: {asset} - {error_msg}")
                except (KeyError, TypeError, AttributeError):
                    pass  # Logged above, continue
            elif status == "err":
                ...
                try:
                    ...
                    pass
                logging.error(f"❌ BUY order failed: {asset} - {error_msg}")
        elif isinstance(result, str):
            logging.error(f"❌ BUY order returned error string: {asset} - {result}")
    ...
    async def place_sell_order(self, asset, amount, slippage=0.01):
        ...
        """
        amount = self.round_size(asset, amount)
        logging.info(f"🔄 Placing SELL order: {asset}, size: {amount}, slippage: {slippage}")
        
        result = await self._retry(lambda: self.exchange.market_open(asset, False, amount, None, slippage))
        
        # Log response for debugging
        logging.info(f"📋 SELL order response: {result}")
        ...
        if isinstance(result, dict):
            ...
            if status == "ok":
                ...
                try:
                    ...
                    if isinstance(response_data, dict):
                        ...
                        if statuses and isinstance(statuses[0], dict):
                            ...
                            if "resting" in status_obj:
                                oid = status_obj.get("resting", {}).get("oid")
                                logging.info(f"✅ SELL order placed (resting): {asset}, order_id: {oid}")
                            elif "filled" in status_obj:
                                filled = status_obj.get("filled", {})
                                logging.info(f"✅ SELL order FILLED: {asset}, size: {filled.get('totalSz')}, avg_price: {filled.get('avgPx')}")
                            elif "error" in status_obj:
                                error_msg = status_obj.get("error", "Unknown error")
                                logging.error(f"❌ SELL order error: {asset} - {error_msg}")
                except (KeyError, TypeError, AttributeError):
                    pass  # Logged above, continue
            elif status == "err":
                error_msg = "Unknown error"
                try:
                    ...
                    pass
                logging.error(f"❌ SELL order failed: {asset} - {error_msg}")
        elif isinstance(result, str):
            logging.error(f"❌ SELL order returned error string: {asset} - {result}")
    ...
    async def cancel_all_orders(self, asset):
        ...
        try:
            query_address = self.main_wallet_address or self.wallet.address
            open_orders = await self._retry(lambda: self.info.frontend_open_orders(query_address))
    ...
    async def get_open_orders(self):
        ...
        try:
            query_address = self.main_wallet_address or self.wallet.address
            orders = await self._retry(lambda: self.info.frontend_open_orders(query_address))
    ...
    async def get_recent_fills(self, limit: int = 50):
        ...
        try:
            ...
            if hasattr(self.info, 'user_fills'):
                fills = await self._retry(lambda: self.info.user_fills(query_address))
            elif hasattr(self.info, 'fills'):
                fills = await self._retry(lambda: self.info.fills(query_address))
            else:
                return []
    ...
    async def get_user_state(self):
        ...
        """
        # Use main wallet address if provided (for querying balances),
        # otherwise use API wallet address
        query_address = self.main_wallet_address or self.wallet.address
        state = await self._retry(lambda: self.info.user_state(query_address))
