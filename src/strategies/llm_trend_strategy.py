...
class LLMTrendStrategy(StrategyInterface):
    ...
    def _decide(self, context, assets):
        ...
        system_prompt = (
            ...
            "   - EXCEPTION: Stop loss overrides all hold time rules - if stop loss is breached, exit IMMEDIATELY\n"
            ...
            "4) Cooldown & Minimum Hold Time: After opening or flipping a position, impose a self-cooldown of at least 3 bars of the decision timeframe (e.g., 3×5m = 15m) before another direction change. CRITICAL EXCEPTION: Stop loss ALWAYS overrides hold time - if stop loss is breached, close immediately regardless of hold time. The system will BLOCK premature exits (less than 15 minutes) unless price has moved 2%+ against the position OR stop loss is breached. Do NOT suggest closing positions that are less than 15 minutes old unless there's a STRONG invalidation (5%+ adverse move) OR stop loss is breached. Encode this in exit_plan (e.g., 'cooldown_bars:3 until 2025-10-19T15:55Z'). You must honor your own cooldowns on future cycles, EXCEPT when stop loss is breached.\n"
