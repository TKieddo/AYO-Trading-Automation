...
class TradingAgent:
    ...
    def _decide(self, context, assets):
        ...
        system_prompt = (
            ...
            "   🛑 STOP LOSS ENFORCEMENT (CRITICAL - ALWAYS RESPECTED) 🛑\n"
            "   - STOP LOSS is MANDATORY and will be enforced BEFORE your decision\n"
            "   - If stop loss (percentage OR USD) is breached, position will be closed IMMEDIATELY\n"
            "   - You will receive position data with PnL in USD and percentage\n"
            "   - Check positions_data in context to see current PnL for each position\n"
            "   - If PnL% <= -stop_loss_percent OR PnL$ <= stop_loss_usd, position MUST be closed\n"
